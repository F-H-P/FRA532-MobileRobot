...
class State:
    ...
    def update(self, a, delta):
        ...
        self.rear_x = self.x - ((WB / 2) * math.cos(self.yaw))
        self.rear_y = self.y - ((WB / 2) * math.sin(self.yaw))
        cmd_vel.linear.x = self.v * math.cos(self.yaw)
        cmd_vel.linear.y = self.v * math.sin(self.yaw)
    ...
    def search_target_index(self, state):
        ...
        if self.old_nearest_point_index is None:
            ...
            distance_this_index = state.calc_distance(self.cx[ind],
                                                      self.cy[ind]) #current position change point cx and cy!!!!
            while True:
                distance_next_index = state.calc_distance(self.cx[ind + 1],
                                                          self.cy[ind + 1])
                if distance_this_index < distance_next_index:
                    break # should removed
                ind = ind + 1 if (ind + 1) < len(self.cx) else ind
                distance_this_index = distance_next_index #should removed
            self.old_nearest_point_index = ind
...
class PathTracking(Node):
    ...
    def goal_path_callback(self, request, response):
        self.path_x = request.x_path
        self.path_y = request.y_path
    ...
    def process(self):
        ...
        target_speed = 0.5  # [m/s] ##! max robot speed

        # initial state
        state = State(x=cx[0], y=cy[0], yaw=0.0, v=0.0) ##! change to initial state
        lastIndex = len(cx) - 1
        time = 0.0
        states = States()
